...
void ExpressionStackShow(struct ExpressionStack *StackTop)
{
    ...
    while (StackTop != NULL)
    {
        ...
        { 
            ...
            {
                case TypeVoid:      printf("vacio"); break;
                case TypeInt:       printf("%d:entero", StackTop->Val->Val->Integer); break;
                case TypeShort:     printf("%d:entero pequeño", StackTop->Val->Val->ShortInteger); break;
                case TypeChar:      printf("%d:caracter", StackTop->Val->Val->Character); break;
                case TypeLong:      printf("%d:entero largo", StackTop->Val->Val->LongInteger); break;
                case TypeUnsignedShort: printf("%d:entero pequeño sin signo", StackTop->Val->Val->UnsignedShortInteger); break;
                case TypeUnsignedInt: printf("%d:entero sin signo", StackTop->Val->Val->UnsignedInteger); break;
                case TypeUnsignedLong: printf("%d:entero largo sin signo", StackTop->Val->Val->UnsignedLongInteger); break;
                case TypeFP:        printf("%f:real", StackTop->Val->Val->FP); break;
                case TypeFunction:  printf("%s:funcion", StackTop->Val->Val->Identifier); break;
                ...
                case TypePointer:
                    ...
                    break;
                case TypeArray:     printf("arreglo"); break;
                case TypeStruct:    printf("%s:estructura", StackTop->Val->Val->Identifier); break;
                case TypeUnion:     printf("%s:union", StackTop->Val->Val->Identifier); break;
                case TypeEnum:      printf("%s:enumeracion", StackTop->Val->Val->Identifier); break;
                case Type_Type:     PrintType(StackTop->Val->Val->Typ, CStdOut); printf(":tipo"); break;
...
long ExpressionAssignInt(struct ParseState *Parser, struct Value *DestValue, long FromInt, int After)
{
    ...
    if (!DestValue->IsLValue) 
        ProgramFail(Parser, "no puedo asignar a esto"); 
...
void ExpressionStackPushDereference(struct ParseState *Parser, struct ExpressionStack **StackTop, struct Value *DereferenceValue)
{
    ...
    if (DerefDataLoc == NULL)
        ProgramFail(Parser, "desreferencia a un puntero Nulo");
...
void ExpressionAssignToPointer(struct ParseState *Parser, struct Value *ToValue, struct Value *FromValue, const char *FuncName, int ParamNo, int AllowPointerCoercion)
{
    ...
    else if (AllowPointerCoercion && FromValue->Typ->Base == TypePointer)
    {
        ...
        AssignFail(Parser, "%t de %t", ToValue->Typ, FromValue->Typ, 0, 0, FuncName, ParamNo); 
}
...
void ExpressionAssign(struct ParseState *Parser, struct Value *DestValue, struct Value *SourceValue, int Force, const char *FuncName, int ParamNo, int AllowPointerCoercion)
{
    ...
    if (IS_NUMERIC_COERCIBLE(DestValue) && !IS_NUMERIC_COERCIBLE_PLUS_POINTERS(SourceValue, AllowPointerCoercion))
        AssignFail(Parser, "%t de %t", DestValue->Typ, SourceValue->Typ, 0, 0, FuncName, ParamNo); 
    ...
    switch (DestValue->Typ->Base)
    {
        ...
        case TypeArray:
            ...
            if (DestValue->Typ->ArraySize != SourceValue->Typ->ArraySize)
                AssignFail(Parser, "de un arreglo de tamano %d a un arreglo de tamano %d", NULL, NULL, DestValue->Typ->ArraySize, SourceValue->Typ->ArraySize, FuncName, ParamNo);
        ...
        case TypeUnion:
            if (DestValue->Typ != SourceValue->Typ)
                AssignFail(Parser, "%t a %t", DestValue->Typ, SourceValue->Typ, 0, 0, FuncName, ParamNo); 
...
void ExpressionQuestionMarkOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, struct Value *BottomValue, struct Value *TopValue)
{
    if (!IS_NUMERIC_COERCIBLE(TopValue))
        ProgramFail(Parser, "el primer argumento a '?' debe ser un numero");
    ...
    switch (Op)
    {
        case TokenAmpersand:
            if (!TopValue->IsLValue)
                ProgramFail(Parser, "no puedo obtener la direccion de esto.");
        ...
        default:
            ...
            if (TopValue->Typ == &FPType)
            {
                ...
                switch (Op)
                {
                    ...
                    default:                ProgramFail(Parser, "operacion invalida"); break;
                }
            ...
            else if (TopValue->Typ->Base == TypePointer)
            {
                ...
                if (TopValue->Val->Pointer == NULL)
                    ProgramFail(Parser, "uso no valido de un puntero Nulo");
                
                if (!TopValue->IsLValue) 
                    ProgramFail(Parser, "no puedo asignar esto"); 
                ...
                ProgramFail(Parser, "operacion invalida");
            break;
    }
}
...
void ExpressionPostfixOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Op, struct Value *TopValue)
{
    ...
    if (IS_NUMERIC_COERCIBLE(TopValue))
    {
        ...
        switch (Op)
        {
            ...
            case TokenRightSquareBracket:   ProgramFail(Parser, "no soportado"); break;  /* XXX */
            case TokenCloseBracket:         ProgramFail(Parser, "no soportadod"); break;  /* XXX */
            default:                        ProgramFail(Parser, "operacion invalida"); break;
        }
    ...
    else if (TopValue->Typ->Base == TypePointer)
    {
        ...
        if (TopValue->Val->Pointer == NULL)
            ProgramFail(Parser, "uso invalido de un puntero Nulo");
...
void ExpressionInfixOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Op, struct Value *BottomValue, struct Value *TopValue)
{
    ...
    if (BottomValue == NULL || TopValue == NULL)
        ProgramFail(Parser, "expresion invalida");
    ...
    { 
        ...
        if (!IS_NUMERIC_COERCIBLE(TopValue))
            ProgramFail(Parser, "el indice del arreglo debe ser un entero");
...
void ExpressionGetStructElement(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Token)
{
    ...
    if (LexGetToken(Parser, &Ident, TRUE) != TokenIdentifier)
        ProgramFail(Parser, "se necesita una miembro de estructura o union luego de '%s'", (Token == TokenDot) ? "." : "->");
    ...
    { 
        ...
        if (StructType->Base != TypeStruct && StructType->Base != TypeUnion)
            ProgramFail(Parser, "no puedo usar '%s' en algo que no es una structura o union %s : es un %t", (Token == TokenDot) ? "." : "->", (Token == TokenArrow) ? "pointer" : "", ParamVal->Typ);
            
        if (!TableGet(StructType->Members, Ident->Val->Identifier, &MemberValue, NULL, NULL, NULL))
            ProgramFail(Parser, "no tiene un miembro llamado '%s'", Ident->Val->Identifier);
...
int ExpressionParse(struct ParseState *Parser, struct Value **Result)
{
    ...
    do
    {
        ...
        { 
            ...
            { 
                ...
                if (OperatorPrecedence[(int)Token].PrefixPrecedence == 0)
                    ProgramFail(Parser, "no esperaba un operador aqui");
                ...
                { 
                    ...
                    if (IS_TYPE_TOKEN(BracketToken) && (StackTop == NULL || StackTop->Op != TokenSizeof) )
                    {
                        ...
                        if (LexGetToken(Parser, &LexValue, TRUE) != TokenCloseBracket)
                            ProgramFail(Parser, "corchetes mal cerrados");
        ...
        { 
            ...
            if (!PrefixState)
                ProgramFail(Parser, "no esperaba un identificador aqui");
        ...
        { 
            ...
            if (!PrefixState)
                ProgramFail(Parser, "valor no esperado aqui");
        ...
        else if (IS_TYPE_TOKEN(Token))
        {
            ...
            if (!PrefixState)
                ProgramFail(Parser, "tipo de dato no esperado aqui");
...
void ExpressionParseMacroCall(struct ParseState *Parser, struct ExpressionStack **StackTop, const char *MacroName, struct MacroDef *MDef)
{
    ...
    { 
        ...
        if (ParamArray == NULL)
            ProgramFail(Parser, "me quede sin memoria x_x");
    }
    else
        ExpressionPushInt(Parser, StackTop, 0);
    ...
    do {
        if (ExpressionParse(Parser, &Param))
        {
            ...
            { 
                if (ArgCount < MDef->NumParams)
                    ...
                    ProgramFail(Parser, "demasiados argumentos para %s()", MacroName);
            }
        ...
        { 
            ...
            if (!TokenCloseBracket)
                ProgramFail(Parser, "mal argumento");
        }
    ...
    { 
        ...
        if (ArgCount < MDef->NumParams)
            ProgramFail(Parser, "faltan argumentos para '%s'", MacroName);
        
        if (MDef->Body.Pos == NULL)
            ProgramFail(Parser, "'%s' no esta definido", MacroName);
...
void ExpressionParseFunctionCall(struct ParseState *Parser, struct ExpressionStack **StackTop, const char *FuncName, int RunIt)
{
    ...
    { 
        ...
        if (FuncValue->Typ->Base != TypeFunction)
            ProgramFail(Parser, "%t no es una funcion - no la puedo llamar", FuncValue->Typ);
            ...
            { 
                ...
                else
                {
                    if (!FuncValue->Val->FuncDef.VarArgs)
                        ProgramFail(Parser, "demasiados argumentos para %s()", FuncName);
                }
            }
    ...
    { 
        /* run the function */
        if (ArgCount < FuncValue->Val->FuncDef.NumParams)
            ProgramFail(Parser, "faltan argumentos para '%s'", FuncName);
        ...
        { 
            ...
            if (FuncValue->Val->FuncDef.Body.Pos == NULL)
                ProgramFail(Parser, "'%s' no esta definida", FuncName);
            ...
            if (ParseStatement(&FuncParser, TRUE) != ParseResultOk)
                ProgramFail(&FuncParser, "esperaba el cuerpo de la funcion");
        
            if (RunIt)
            {
                if (FuncParser.Mode == RunModeRun && FuncValue->Val->FuncDef.ReturnType != &VoidType)
                    ProgramFail(&FuncParser, "no se devolvio nungun valor de una funcion de tipo %t", FuncValue->Val->FuncDef.ReturnType);

                else if (FuncParser.Mode == RunModeGoto)
                    ProgramFail(&FuncParser, "no encuentro la etiquieta goto '%s'", FuncParser.SearchGotoLabel);
            }
...
long ExpressionParseInt(struct ParseState *Parser)
{
    ...
    { 
        if (!IS_NUMERIC_COERCIBLE(Val))
            ProgramFail(Parser, "esperaba un valor entero en vez de un %t", Val->Typ);
